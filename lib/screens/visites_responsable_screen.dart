...
import 'package:visite_securite/screens/ResponsableActionScreen.dart';
import '../services/api_service.dart';

class VisitesResponsableScreen extends StatefulWidget {
  final int responsableId;

  const VisitesResponsableScreen({super.key, required this.responsableId});

  @override
  _VisitesResponsableScreenState createState() => _VisitesResponsableScreenState();
}

class _VisitesResponsableScreenState extends State<VisitesResponsableScreen> {
  ...
  Future<void> _loadVisites() async {
    try {
      ...
      final data = await ApiService.fetchVisitesByResponsable(widget.responsableId);
      setState(() {
        _visites = data;
        _isLoading = false;
      });
        ...
        return Card(
          ...
          child: ListTile(
            ...
            onTap: () {
              ...
              if (etat == 'cloture' || etat == 'realise') {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => ResponsableActionScreen(visite: visite),
                  ),
                );
              } else {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text("Vous ne pouvez accéder aux actions que si la visite est clôturée ou réalisée."),
