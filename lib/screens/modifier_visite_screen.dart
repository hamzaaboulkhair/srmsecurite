...
class ModifierVisiteScreen extends StatefulWidget {
  final Map<String, dynamic> visite;

  const ModifierVisiteScreen({super.key, required this.visite});

  @override
  _ModifierVisiteScreenState createState() => _ModifierVisiteScreenState();
}

class _ModifierVisiteScreenState extends State<ModifierVisiteScreen> {
  ...
  void _initializeVisite() {
    selectedDate = DateTime.parse(widget.visite["date"] as String);
    selectedHeureDebut = _parseTime(widget.visite["heureDebut"] as String);
    selectedHeureFin = _parseTime(widget.visite["heureFin"] as String);
    participants = List<String>.from(
        (widget.visite["participants"] as List<dynamic>?)?.map((p) => p["nom"] as String).toSet() ?? []
    );
  }
  ...
  void _submitModification() async {
    ...
    final formattedHeureDebut = "${selectedHeureDebut!.hour.toString().padLeft(2, '0')}:${selectedHeureDebut!.minute.toString().padLeft(2, '0')}";
    final formattedHeureFin = "${selectedHeureFin!.hour.toString().padLeft(2, '0')}:${selectedHeureFin!.minute.toString().padLeft(2, '0')}";
    ...
    final success = await ApiService.modifierVisite(widget.visite["id"] as int, visiteData);
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("Modifier les Participants", style: GoogleFonts.montserrat(fontSize: 18, fontWeight: FontWeight.w500, color: Colors.blueGrey.shade800)),
              ...
              children: [
                  ...
                  child: Material(
                    ...
                    child: DropdownButtonFormField<String>(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintStyle: GoogleFonts.montserrat(fontSize: 14, color: Colors.grey.shade600),
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    } else if (selectedParticipant != null && participants.contains(selectedParticipant)) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Ce participant est déjà ajouté")),
                      );
                    }
                  },
                  style: ElevatedButton.styleFrom(
